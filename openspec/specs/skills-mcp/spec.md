...
`list_skills(registry: str)` SHALL return a list of skill name strings available in
the named registry. For GitHub registries it lists subdirectory names in the configured
`skills_dir`. For HTTP registries it returns a single-element list containing the
declared `skill_name`. Names only are returned — no descriptions, no metadata —
to avoid N+1 upstream API calls.

#### Scenario: Lists GitHub registry skills
- **WHEN** `list_skills` is called for a GitHub registry whose `skills_dir` contains three subdirectories
- **THEN** a list of exactly three skill names is returned
...
The GitHub adapter SHALL fetch skill content from a multi-skill GitHub repository
(one skill per subdirectory inside a configured `skills_dir`) locked to a configured
`ref` (branch, tag, or full/abbreviated commit SHA). It SHALL enumerate skill
subdirectories via the GitHub Contents API. It SHALL enumerate a skill's file tree via
the recursive Git Trees API (one request) and fetch individual file contents via the
Git Blobs API with `Accept: application/vnd.github.raw` (supporting files larger than
1 MB). When a `list_skills` result is truncated by the GitHub API's 1,000-entry limit,
the server SHALL log a warning to stderr.

#### Scenario: Lists skill subdirectories from GitHub
- **WHEN** `list_skills` is called for a GitHub registry
- **THEN** the adapter calls the Contents API on `skills_dir` at `ref` and returns subdirectory names

#### Scenario: Fetches SKILL.md via raw blob
- **WHEN** `get_skill` is called for a GitHub skill
- **THEN** the adapter uses the recursive Trees API to locate `SKILL.md` and fetches it via the Blobs API
