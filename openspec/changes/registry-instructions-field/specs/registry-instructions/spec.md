...
`_build_list_registries_description(cfg: Config) -> str` that assembles the
`list_registries` tool description from the loaded configuration. The assembled
string SHALL begin with the current static introductory text, then append one
entry per configured registry in config order. Each entry SHALL include the
registry name. When the registry has a non-None `description`, it SHALL be
included. When the registry has a non-None `instructions`, it SHALL be appended
to that registry's entry. Registries with no `instructions` SHALL appear without
a call-to-action. The `list_registries` tool SHALL be registered using the result
of this function as its `description=` parameter. The `list_skills` and `get_skill`
tool descriptions SHALL remain unchanged.

#### Scenario: Description with no registries configured
- **WHEN** `_build_list_registries_description` is called with a config containing no registries
...
- **WHEN** `_build_list_registries_description` is called with a registry that has both `description` and `instructions` set
...
- **WHEN** `_build_list_registries_description` is called with a registry that has `description` set but `instructions == None`
...
- **WHEN** `_build_list_registries_description` is called with a registry where both `description` and `instructions` are `None`
...
- **WHEN** `_build_list_registries_description` is called with a registry that has `instructions` set but `description == None`
...
- **WHEN** `_build_list_registries_description` is called with two registries
